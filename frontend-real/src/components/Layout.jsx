...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-navy-950 flex flex-col">
      ...
      <header
        className={`animate-fade-in sticky top-0 z-50 isolate transition-[background-color,box-shadow,border-color] duration-300 ease-out ${
          scrolled
            ...
            <nav className="flex items-center gap-1">
              {isAdminRoute && user?.role === "ADMIN" ? (
                <>
                  ...
                  <NavLink to="/admin/mentors" className={navLinkClass}>
      ...
      <main className="animate-fade-in-up flex-1 w-full px-4 sm:px-6 lg:px-8 py-6 pb-16">
        <Outlet />
      </main>
      ...
      <footer className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[0.06] bg-navy-900/80 backdrop-blur-xl">
        <div className="mx-auto flex h-11 w-full max-w-[1600px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          {showAvailabilityLegend ? (
            ...
            <span className="text-xs text-ink-600">Mentorque · Mentoring Scheduler</span>
          )}
        </div>
      </footer>
    </div>
